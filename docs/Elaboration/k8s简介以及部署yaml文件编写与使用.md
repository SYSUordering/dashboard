...
3. kubelet是API Server和Minion之间的桥梁，接收APIServer分配给它的commands和work，与持久性键值存储etcd、file、server和http进行交互，读取配置信息。  
4. 具体：
...
**Statefulset**：statefulset创建时并没有创建ReplicaSets。除此之外，deployment和statefulset创建的pod在命名上也有很大的不同，statefulset的pod都是statefulset的name-#，其中#是0,1,2...有序的，而deployment的是deployment的name-str，str是一个随机字符串，没有规则。删除后新建的pod名字一样并且域名一样（ip不一样），deployment创建的不一样。  
